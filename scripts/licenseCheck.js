...
const allowlistData = JSON.parse(
  fs.readFileSync("scripts/licenseAllowList.json", "utf8"),
);
const allLicenseData = JSON.parse(
  fs.readFileSync("scripts/allLicenseResults.json", "utf8"),
);
...
const isLicenseAllowed = (license) => {
  ...
  if (
    !sanitizedLicense.includes(" OR ") &&
    !sanitizedLicense.includes(" AND ")
  ) {
  ...
  if (sanitizedLicense.includes(" OR ")) {
    const licenseParts = sanitizedLicense
      .split(" OR ")
      .map((part) => part.trim());
...
if (Object.keys(nonAllowedDependencies).length > 0) {
  ...
  console.error(
    "Error: There are dependencies with non-allowed licenses. Failing the job.",
  );
  process.exit(1);
} else {
