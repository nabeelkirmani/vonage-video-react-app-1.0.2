...
describe("createArchiveFromServer", () => {
  ...
  it("should return status as failed for failed and expired archives", () => {
    ...
    expect(
      createArchiveFromServer({ ...failedServerArchive, status: "expired" })
        .status,
    ).toBe("failed");
  });

  it("should return available as failed for status available archives", () => {
    expect(createArchiveFromServer(availableServerArchive).status).toBe(
      "available",
    );
  });

  it("should return status as pending for started, stopped, uploaded, and paused archives", () => {
    expect(createArchiveFromServer(startedServerArchive).status).toBe(
      "pending",
    );
    expect(
      createArchiveFromServer({ ...startedServerArchive, status: "stopped" })
        .status,
    ).toBe("pending");
    expect(
      createArchiveFromServer({ ...startedServerArchive, status: "uploaded" })
        .status,
    ).toBe("pending");
    expect(
      createArchiveFromServer({ ...startedServerArchive, status: "paused" })
        .status,
    ).toBe("pending");
  });
});
