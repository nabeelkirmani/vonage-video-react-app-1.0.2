...
describe("ArchiveList", () => {
  it("should display message if there are no archives", () => {
    ...
    expect(
      screen.getByText("There are no recordings for this meeting"),
    ).toBeVisible();
  });
  ...
  it("should render a download button for available archives", () => {
    ...
    const listItem = screen.getByTestId(
      `archive-list-item-${availableArchive.id}`,
    );
    expect(
      within(listItem).getByTestId("archive-download-button"),
    ).toBeVisible();
  ...
  it("should render an error icon for failed archives", () => {
    render(<ArchiveList archives={testArchives} />);
    const listItem = screen.getByTestId(
      `archive-list-item-${failedArchive.id}`,
    );
  ...
  it("should render a spinner for pending archives", () => {
    render(<ArchiveList archives={testArchives} />);
    const listItem = screen.getByTestId(
      `archive-list-item-${pendingArchive.id}`,
    );
    expect(
      within(listItem).getByTestId("archive-loading-spinner"),
    ).toBeVisible();
  });
});
