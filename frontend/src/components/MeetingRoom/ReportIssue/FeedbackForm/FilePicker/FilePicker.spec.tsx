...
describe("FilePicker component", () => {
  ...
  it("uploads and previews a valid image file", async () => {
    ...
    ); // Check that the src is a data URL
  ...
  it("shows an error if the file is too large", async () => {
    ...
    expect(
      await screen.findByText(/The maximum upload size is 20MB/i),
    ).toBeInTheDocument();
  });
  ...
  it("takes a screenshot when Capture screenshot button is clicked", async () => {
    ...
    (navigator.mediaDevices.getDisplayMedia as Mock).mockResolvedValue(
      mockStream,
    );
