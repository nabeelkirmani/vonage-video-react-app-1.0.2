...
vi.mock("../../../utils/util", async () => {
  const actual = await vi.importActual<typeof import("../../../utils/util")>(
    "../../../utils/util",
  );
  return {
    ...actual,
...
describe("AudioInputOutputDevice Component", () => {
    ...
    Object.defineProperty(global.navigator, "mediaDevices", {
      ...
      value: {
        ...
        removeEventListener: vi.fn((event, listener) =>
          deviceChangeListener.off(event, listener),
        ),
      },
    });
  });
  ...
  it("renders the output devices if the browser supports setting audioOutput device", async () => {
    ...
    await waitFor(() =>
      expect(outputDevicesElement.children).to.have.length(3),
    );
    expect(outputDevicesElement.firstChild).toHaveTextContent("System Default");
    expect(
      (outputDevicesElement.firstChild as HTMLOptionElement).selected,
    ).toBe(true);
    expect(outputDevicesElement.children[1]).toHaveTextContent(
      "Soundcore Life A2 NC (Bluetooth)",
    );
    expect(outputDevicesElement.children[2]).toHaveTextContent(
      "MacBook Pro Speakers (Built-in)",
    );

    await act(() =>
      (outputDevicesElement.children[2] as HTMLOptionElement).click?.(),
    );

    expect(mockSetAudioOutputDevice).toHaveBeenCalledWith(
      audioOutputDevices[2].deviceId,
    );
    await expect(
      (outputDevicesElement.children[2] as HTMLOptionElement).selected,
    ).toBe(true);
  });
  ...
  it("renders the default output device if the browser does not support setting audioOutput device", async () => {
    ...
    mockGetAudioOutputDevices.mockResolvedValue([
      vonageDefaultEmptyOutputDevice,
    ]);
    render(
      <AudioOutputProvider>
        <AudioInputOutputDevices
    ...
    const outputDevicesElement = screen.getByTestId("output-devices");
    await waitFor(() =>
      expect(outputDevicesElement.children).to.have.length(1),
    );
    expect(outputDevicesElement.firstChild).toHaveTextContent("System Default");
    expect(
      (outputDevicesElement.firstChild as HTMLOptionElement).selected,
    ...
    await act(() =>
      (outputDevicesElement.firstChild as HTMLOptionElement).click?.(),
    );
  ...
  it("updates audio output device list if device is removed", async () => {
    ...
    // select device 2
    await act(() =>
      (outputDevicesElement.children[1] as HTMLOptionElement).click?.(),
    );
    expect(mockSetAudioOutputDevice).toHaveBeenCalledWith(
      audioOutputDevices[1].deviceId,
    );
    await expect(
      (outputDevicesElement.children[1] as HTMLOptionElement).selected,
    ).toBe(true);
    ...
    await waitFor(() =>
      expect(outputDevicesElement.children).to.have.length(2),
    );
    expect(outputDevicesElement.firstChild).toHaveTextContent("System Default");
    expect(
      (outputDevicesElement.firstChild as HTMLOptionElement).selected,
    ...
    const removedDevice = queryByText(
      outputDevicesElement,
      "Soundcore Life A2 NC (Bluetooth)",
    );
