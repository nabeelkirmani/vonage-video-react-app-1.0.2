...
describe("ChatToggleButton", () => {
  it("should show unread message number", () => {
    ...
    expect(screen.getByTestId("chat-toggle-unread-count").textContent).toBe(
      "10",
    );
  });
  ...
  it("should not show unread message number when number is 0", () => {
    ...
    const badge = within(
      screen.getByTestId("chat-toggle-unread-count"),
    ).getByText("0");
  ...
  it("should have a white icon when the list is closed", () => {
    ...
    expect(screen.getByTestId("ChatIcon")).toHaveStyle(
  ...
  it("should invoke callback on click", () => {
    ...
    render(
      <ChatToggleButton handleClick={handleClick} isOpen unreadCount={10} />,
    );
